...
def create_streaming_dataloader(dataset,
    ...
    if dataset.startswith('/'):
        # Local file path - load from JSON.gz files
        data_files = f"{dataset}/*.json.gz"
        # For local files, always use 'train' split and handle validation differently
        data = load_dataset("json", data_files=data_files, streaming=True, split='train').shuffle(42+global_rank)
        
        # If validation split is requested, skip some data for validation
        if split == 'validation':
            # Skip first 90% of data for validation (use last 10%)
            data = data.skip(int(0.9 * 1000000))  # Approximate skip for validation
        elif split == 'train':
            # Use first 90% for training
            data = data.take(int(0.9 * 1000000))  # Approximate take for training
    else:
        # HuggingFace dataset - use original logic
        data = load_dataset(dataset, name=name, streaming=True, split=split).shuffle(42+global_rank)
