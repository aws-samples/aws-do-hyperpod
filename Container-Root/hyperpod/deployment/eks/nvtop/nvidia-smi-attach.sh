...
help(){
	...
	echo "       executes 'watch nvidia-smi' in the specified nvtop pod"
	...
	echo ""
	echo "       nvtop_pod_name - name or partial (unique substring) name of pod to run 'watch nvidia-smi' in"
	echo ""
}
...
if [ "$1" == "--help" ]; then
	...
	CMD="kubectl exec -it ${pod_name} -- watch nvidia-smi"
